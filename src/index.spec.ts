...
import { dotProduct, dotProductJS, dotProductWasm } from "./";
...
test("perf: Measure and report the performance of 20000 single, JIT optimized JS-based dot product calculations", async () => {
...
test("perf: Measure and report the performance of 20000 single, SIMD-optimized WASM-based dot product calculations", async () => {
  const Module = await getWasmModule();
  const times: { [index: number]: number } = {};
  const iterations = 20000;
  const dimensions = [4, 384, 1024];
...
test("perf: Measure and report the performance of 20000 single, JIT-optimized JS-based dot product calculations, passed at once", async () => {
  const times: { [index: number]: number } = {};
  const iterations = 1;
  const dimensions = [4, 384, 1024];
  await perf(
...
test("perf: Measure and report the performance of 20000 single, SIMD-optimized WASM-based dot product calculations, passed at once", async () => {
  const Module = await getWasmModule();
  const times: { [index: number]: number } = {};
  const iterations = 1;
  const dimensions = [4, 384, 1024];
